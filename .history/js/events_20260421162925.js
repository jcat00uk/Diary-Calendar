...
export function generateOccurrencesForSeries(series, dateKey) {
  ...
  const count = countOccurrencesBefore(series, dateKey);
  ...
  if (series.exceptions[dateKey]) return series.exceptions[dateKey]; // overridden occurrence
...
export function getAllDatesForSeries(data, seriesId) {
  ...
  for (const dateKey in data.days) {
    ...
    if (events.some(ev => (ev.seriesId || ev.id) === seriesId)) {
      dates.push(dateKey);
    }
  }
  return dates;
