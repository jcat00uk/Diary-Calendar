...
export function initFormatToolbar() {
  ...
  () => {
    ...
    if (sel && sel.rangeCount > 0) {
      ...
      if (span && span.style && span.style.backgroundColor &&
          span.style.backgroundColor.toLowerCase() === _lastHiColor.toLowerCase()) {
  ...
  const openHiPicker = e => {
    ...
    _showHiColorPicker(hiPickBtn, c => {
      ...
      if (sel && sel.rangeCount > 0) {
        ...
        const span = range.startContainer.parentElement;
        if (span && span.style && span.style.backgroundColor &&
            span.style.backgroundColor.toLowerCase() === c.toLowerCase()) {
          _exec('backColor', 'transparent');
        } else {
          _exec('backColor', c);
        }
      } else {
