...
const repeats = data.series
    .map(series => generateOccurrencesForSeries(data, series, dateKey))
    .filter(x => x !== null);
...
function countOccurrencesBefore(data, series, targetKey) {
  let count = 0;

  for (const dateKey in data.days) {
    if (dateKey >= targetKey) continue;
    if (generateOccurrencesForSeries(data, series, dateKey)) {
      count++;
    }
  }
